...
func Comment(c *gin.Context) {
	...
	if actionType == 1 {
		...
		comment, _ = CommentLeaving(videoId, userId, commentText)
	} else if actionType == 2 {
		commentId := c.Query("comment_id")
		comment, _ = CommentDeleting(videoId, userId, commentId)
	} else {
		// 返回异常响应信息
...
func CommentLeaving(videoId string, userId string, commentText string) (CommentResponse, error) {
	...
	// 添加评论
	commentId, createDate, _ := service.AddComment(commentText, userId, videoId)
